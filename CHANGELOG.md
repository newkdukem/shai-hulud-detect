...
### Added
- **Historical forensics mode (`--history`)**: the existing checks answer *"is this tree infected NOW?"* — a worm that was installed, ran, and was cleaned up afterwards (lockfile fixed, payload deleted, branch force-pushed away) leaves no live artifact and scanned clean. `--history` closes that gap by walking every repo's git object history (all refs, plus reflog-only and unreachable commits) for evidence of a PAST infection. Six independent signals, all reported as **HIGH** (a clean tree today does not un-steal the credentials that existed during the infection window):
  - **Compromised versions in historical manifests** (`check_git_history`): one bounded `log --raw` walk enumerates every distinct `package.json` / `package-lock.json` / `yarn.lock` / `pnpm-lock.yaml` / `requirements*.txt` blob ever committed; each unique blob is parsed once (reusing the live parsers' awk logic, including the inline-JSON explode fix and pnpm transform) and checked against the compromised map (npm + PyPI).
  - **IOC filenames that were later deleted**: pathspec walk for the campaign artifact inventory (`shai-hulud-workflow.yml`, `router_init.js`, `setup_bun.js`, dead-man's-switch scripts, obfuscated exfil JSONs, `formatter_*.yml` workflows, ...). Names too generic to be meaningful as a pathname-only historical signal (`index.js`, `cat.py`, `data.json`) are deliberately excluded.
  - **Known-malicious blobs**: historical contents at payload-shaped paths (campaign artifact names plus `bundle.js` / `setup.mjs` / `setup.cjs` / `setup.pth` / `binding.gyp` / `math_init.js` / `jia.js` / `art.js` / `cat.py`) are SHA-256'd against `MALICIOUS_HASHLIST` — catches a payload that was committed and later deleted or replaced, since the blob object survives in the odb.
  - **Shai-Hulud refs and reflog traces**: `for-each-ref` plus the HEAD reflog, which remembers checkouts of the worm's `shai-hulud` staging branch even after the branch was deleted. Plain `-migration` branch names are NOT matched (ordinary repos have legitimate migration branches).
  - **Campaign IoC strings (pickaxe)**: `-S` walks for nine high-signal literals (original webhook.site exfil UUID, `npm-cache.com`, wipe-threat token description, Mini Shai-Hulud markers, the reversed and forward "Here We Go Again" markers, `audit.checkmarx.cx`, `RevokeAndItGoesKaboom`) — fires on the commit that introduced an IoC *and* the one that cleaned it up.
  - **Lost commits carrying IOCs**: commits reachable only from reflogs (force-push, amend, reset) or fully unreachable (`fsck --connectivity-only --unreachable`) get their trees and subjects checked for IOC names — exactly the shape of an infection that was "cleaned up" by rewriting history. Bare rewritten-history evidence (normal for rebase workflows) is reported as an informational note only and never affects the exit code.
- **Package-manager cache forensics (`check_package_caches`)**: with `--history --check-host` (host-level consent required, matching the `--check-host` convention), user-level caches are checked for compromised versions: the `~/.npm/_cacache/index-v5` tarball-URL index, yarn classic (`~/.cache/yarn`, macOS `~/Library/Caches/Yarn`) and yarn berry (`~/.yarn/berry/cache`) entries, the bun install cache, and pip wheel caches (PEP 427 name normalization). A cached compromised tarball proves that version was **downloaded on this machine** at some point, regardless of what any project references today. pnpm's store is content-addressed (no names on disk) and is covered indirectly via historical `pnpm-lock.yaml` blobs.
- **`--history-depth N`** (default 1000, `0` = unlimited): bounds every history walk. Because `-n` alone caps *displayed* commits rather than traversal (pathspec/pickaxe walks would still crawl full history hunting for the Nth match), the committer date of the oldest in-budget commit is also passed as `--since`, which actually stops the walk. Per-repo object budgets (500 manifest blobs parsed, 300 blobs hashed, 100 lost commits tree-scanned) keep one pathological repo from stalling the scan.
- **Reporting**: five new HIGH report sections (with a dedicated past-infection remediation order: dead-man's-switch check before rotation, npm/GitHub audit-log review from the commit dates shown, cache purge), `--save-log` HIGH entries, and `--json` findings for all five history sources. `--bulk` propagates `--history`/`--history-depth` to every per-project scan (cache forensics stays per-machine and is not repeated per project).
- **Test coverage**: `run-tests.sh` gains a `--history` block that builds a git fixture on the fly (nested `.git` dirs can't live in `test-cases/`): a repo whose working tree is clean but whose history contains a compromised `package.json` / `yarn.lock` / scoped `pnpm-lock.yaml` / `requirements.txt`, a deleted IOC workflow file, a deleted `shai-hulud` branch, a reset-away commit carrying `router_init.js`, a committed-then-removed wipe-threat string, and a deleted blob whose SHA-256 is in `MALICIOUS_HASHLIST` — asserting the repo scans clean WITHOUT `--history`, exits 1 with it, and that each signal fires; plus cache-forensics assertions against a seeded fake `$HOME` (npm index, yarn berry zip, pip wheel), the cache-skip notice without `--check-host`, and a live scoped-pnpm lockfile regression guard. Suite: 238 → 256 checks.

### Fixed
- **`git_repos.txt` contained raw `/.git` paths, silently disabling repo-dir checks** (`collect_all_files`): `find ... || true | sed 's|/.git$||'` parses as `find || { true | sed; }`, so the `sed` never ran and the inventory held `/path/repo/.git` instead of `/path/repo`. Consumers that append `/.git/...` to each entry — `check_git_branches` (shai-hulud branch names) and `check_shai_hulud_repos` (repo names, `-migration` pattern, git remotes, `data.json`) — tested paths like `/path/repo/.git/.git/refs/heads` and never fired on the primary path. The pipeline is now grouped correctly.
- **Lockfile block parser missed scoped packages in pnpm pseudo-locks and package-lock v1** (`check_package_integrity`, surfaced while building the `--history` parsers, fixed in both): the fallback key pattern `"[^"\/]+":` rejected any name containing `/`, so `"@ctrl/tinycolor": {` entries produced by `transform_pnpm_yaml` (and scoped entries in package-lock v1 `dependencies`) were never checked — only unscoped names matched. Additionally the `"packages": {` wrapper itself matched the fallback and opened a bogus block, which swallowed the FIRST package entry of every pnpm pseudo-lock. The pattern now admits `/` (the `node_modules/` rule still claims v2/v3 keys first, via `next`) and `packages` joined the structural-key exclusion list.

### Changed
- **`shai-hulud-detector.sh`**: `SCRIPT_VERSION` 3.14.1 → 3.15.0.
- **`README.md`**: new "Historical forensics (`--history`)" scenario section, flags table rows, "How it works" step 6; tests badge/count 238 → 256.
